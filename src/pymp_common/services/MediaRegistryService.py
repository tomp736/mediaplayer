...
class MediaRegistryService():
    ...
    def register(self, service_id, proto, host, port):
        return self.mediaRegistryProvider.register(service_id, proto, host, port)
    ...
    def get_media_service(self, media_id) -> str:
        return self.mediaRegistryProvider.get_media_service(media_id)
    ...
    def check_media_service(self, service_id):
        ...
        media_svc_media_ids = []
        try:
            mediaProvider = ProviderFactory.get_media_provider(service_id)
            if mediaProvider:
                status = mediaProvider.get_status()
                if status:
                    media_svc_media_ids = mediaProvider.get_media_ids()
                    logging.info(
                        f"MediaProvider {service_id} passes status check.")
                else:
                    logging.info(
                        f"MediaProvider {service_id} fails status check.")
                    self.mediaRegistryProvider.remove(service_id)
            else:
                logging.info(f"MediaProvider {service_id} is None.")
    ...
    def check_media_sources(self, service_id, mediaIds):
        ...
        for registryMediaID in redis_svc_media_ids:
            ...
            if not registry_media[registryMediaID] == service_id:
                ...
                self.mediaRegistryProvider.register_media(
        ...
        for media_id in mediaIds:
            ...
            self.mediaRegistryProvider.register_media(media_id, service_id)
